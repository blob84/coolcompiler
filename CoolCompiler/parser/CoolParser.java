...
public class CoolParser {
	...
	private boolean noclass = false;//it is not a class because there is an error in it's definition
	private int lineno = 0;

    public CoolParser(CoolLexer lexer) { //throw Exception {
		this.k = 2;
        this.lexer = lexer;
		...
		//lineno = lexer.curr_lineno();
        for (int i = 0; i < k; i++) 
           advance(); 
    }
    /*
		return the value of the token if it is a string literal, integer literal, boolean literal or identifier,
		return the token name otherwise
	*/
    ...
    public void advance() {
		try {
			lineno = lexer.curr_lineno();	//store the current line number anytime one token is obtained 
	...
	public int error_recovery() {	//skip token until a synchronized token
		if (curr_non_terminal.equals("class"))
			while ( (LL(1).sym != TokenConstants.SEMI || LL(2).sym != TokenConstants.CLASS) && LL(1).sym != TokenConstants.EOF) 
				advance(); 
		else if (curr_non_terminal.equals("feature"))
			while ( (LL(1).sym != TokenConstants.SEMI || LL(2).sym != TokenConstants.OBJECTID) && LL(1).sym != TokenConstants.EOF) 
					advance(); 
		else if (curr_non_terminal.equals("blockexpr"))
			while ( LL(1).sym != TokenConstants.SEMI && LL(1).sym != TokenConstants.EOF) 
					advance(); 
		else if (curr_non_terminal.equals("letexpr"))
			while ( LL(1).sym != TokenConstants.COMMA && LL(1).sym != TokenConstants.IN && LL(1).sym != TokenConstants.EOF) 
				advance(); 
		error = false;
...
/* program ::= classP SEMI { classP SEMI } */
    public Symbol program() {
		...
		do {
			...
			if (s != null) {         
				c = (Class_) s.value;
				classes.appendElement(c); 
			} 
			else { 
				if (noclass) { noclass = false; return null; }
				int token = error_recovery(); //System.out.println("LL(1): "+Utilities.tokenToString(LL(1))); 
				if (token == TokenConstants.EOF) return null; } //class error, go to next class
			match(TokenConstants.SEMI); //match(TokenConstants.EOF_TYPEID);
			if (error) { int token = error_recovery(); if (token == TokenConstants.EOF) return null; } //class error, go to next class
...
/* classP ::= CLASS TYPEID [ INHERITS TYPEID ] LBRACE { feature SEMI } RBRACE 
    ...
    Symbol classP() {
		...
		if (error) { noclass = true; return null; }  
        ...
        while (LL(1).sym == TokenConstants.OBJECTID) { 
			...
			if (s != null) {			
				...
				int token = error_recovery(); 	//System.out.println("linen: "+curr_lineno()+ "	1 LL(1): "+ Utilities.tokenToString(LL(1)));
				if (token == TokenConstants.EOF) return null; //there aren't other features, go to next class
			}	//System.out.println("linen: "+curr_lineno()+ "	1 LL(1): "+ Utilities.tokenToString(LL(1)));
        ...
        if (LL(1).sym == TokenConstants.LPAREN) {	//it is a method
			...
			if (e !=  null)
				expr = (Expression) e.value; 
											...
											//System.out.println("1 LL(1): "+ Utilities.tokenToString(LL(1)));
			match(TokenConstants.RBRACE); 	//System.out.println("2 LL(1): "+ Utilities.tokenToString(LL(1)));
			if (error) return null; 
    ...
    Symbol expr() {
        ...
        if (LL(1).sym == TokenConstants.OBJECTID && LL(2).sym == TokenConstants.ASSIGN) {   
            ...
            switch (LL(1).sym) {
                ...
                default: //System.out.println("line: "+curr_lineno()+"	err rec LL(1): "+ Utilities.tokenToString(LL(1))); 
						syntax_error(LL(1)); 
...
*/
    ...
    Expression otherexpr() {
        ...
        if (LL(1).sym == TokenConstants.LET) {
            ...
            match(TokenConstants.LBRACE);  //System.out.println("LL(1): "+ (String) LL(1).value.toString());
			if (error) return null; 

			while (true) {
				do {	
					Symbol e = expr(); 				//System.out.println("LL(1): "+ Utilities.tokenToString(LL(1)));
					curr_non_terminal = "blockexpr";
					if (e != null) 
						body.appendElement((Expression) e.value); 
					else {	
						//blockerror = true;				
						int token = error_recovery(); //System.out.println("line: "+curr_lineno()+"	err rec LL(1): "+ Utilities.tokenToString(LL(1))); //go to next block expression
						if (token == TokenConstants.EOF) return null; //return if there isn't a next block expression
					}
					match(TokenConstants.SEMI);
					if (error) { 
						//blockerror = true; //System.out.println("line: "+curr_lineno()+"	err rec LL(1): "+ Utilities.tokenToString(LL(1)));
				...
				else if (LL(1).sym == TokenConstants.EOF) return null; 
...
/*				Symbol e = expr();
			...
			}	System.out.println("LL(1): "+ Utilities.tokenToString(LL(1)));
			match(TokenConstants.SEMI);
			if (error) { 
				blockerror = true;
				int token = error_recovery(); //go to next block expression
				if (token == TokenConstants.EOF) return null; //return if there isn't a next block expression
			}	System.out.println("LL(1): "+ (String) LL(1).value.toString());
			//while (true) {
		        while (LL(1).sym == TokenConstants.OBJECTID || LL(1).sym == TokenConstants.INT_CONST || LL(1).sym == TokenConstants.STR_CONST || LL(1).sym == TokenConstants.NOT ||
		             LL(1).sym == TokenConstants.ISVOID || LL(1).sym == TokenConstants.LET || LL(1).sym == TokenConstants.IF || LL(1).sym == TokenConstants.WHILE ||
		             LL(1).sym == TokenConstants.CASE || LL(1).sym == TokenConstants.NEW || LL(1).sym == TokenConstants.BOOL_CONST || 

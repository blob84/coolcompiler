...
protected int numElts;            	// Number of elements in the list
  ...
  public Iterable<Position<E>> positions() {// crea una lista di posizioni
	...
	public void removeFrom(Position<E> p) {
		DNode<E> pp = checkPosition(p);
		while (pp != trailer) {
			Position<E> next = next(p);
			remove(p);
			pp = (DNode<E>) next;
		}
	}
	public void removeTo(Position<E> p) {
		DNode<E> pp = checkPosition(p);
		while (pp != header) {
			Position<E> prev = prev(p);
			remove(p);
			pp = (DNode<E>) prev;
		}
	}
	public PositionList<E> remoFrom(E x) {
		PositionList<E> list = new NodePositionList<E>();
		Position<E> p = first();
		while (p != trailer && p.element() != x) 
			p = next(p);
		while (p != trailer) {
			Position<E> next = next(p);
	...
	public PositionList<E> removeTo(E x) {
		PositionList<E> list = new NodePositionList<E>();
		Position<E> p = last();
		while (p != header && p.element() != x) 
			p = prev(p);
		while (p != trailer) {
			Position<E> prev = prev(p);
			list.addLast(p.element());

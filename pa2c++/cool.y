...
%{
  ...
  for the location of tokens */
    ...
    */
	...
	%type <case_> case;
	%type <expression> expression_semi; /* expression for block {...} expression, following a semicolon */
	%type <expression> letexpr;
	%type <expression> letexpr_init;
    ...
    */
	...
	block_expression_list
		: expression_semi 
			{ $$ = single_Expressions($1); }
		| block_expression_list expression_semi 
	...
	expression_semi 
       ...
       | WHILE expression LOOP expression POOL ';'
	     { $$ = loop($2, $4); }
       | '{' block_expression_list '}' ';'		//current debugging
	     { $$ = block($2); }
	   | letexpr ';'
	     { $$ = $1;  } 
	...
	letexpr	
		: LET OBJECTID ':' TYPEID letexpr_init	
		 { $$ = let($2, $4, no_expr(), $5); }
		 | LET OBJECTID ':' TYPEID ASSIGN expression letexpr_init
		   { $$ = let($2, $4, $6, $7); }
	...
	letexpr_init	
			/* 
				multiple let identifiers are transformed to single let identifier, 
			   	example: let x: Int, y: Int, z: Int in e <=> let x: Int in (let y: Int in (let z: Int in e))) 
			*/
		: ',' OBJECTID ':' TYPEID letexpr_init
		 { $$ = let($2, $4, no_expr(), $5); }
		| ',' OBJECTID ':' TYPEID ASSIGN expression letexpr_init
		  { $$ = let($2, $4, $6, $7); }
		| IN expression
		  { $$ = $2; }
		| error letexpr_init { ; }
